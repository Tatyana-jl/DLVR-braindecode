...
class BatchCntTrainer(object):
        ...
        if n_trials >= self.n_min_trials:
            ...
            for _ in range(self.n_updates_per_break):
                ...
                if self.savegrad:
                    ...
                    torch.save(i_supercrops, 'batchInd_' + file_name)
    ...
    def train_on_batch(self, inputs, targets):
        self.model.train()
        input_vars = Variable(torch.cuda.FloatTensor(inputs))
        target_vars = Variable(torch.cuda.LongTensor(targets))
        if self.cuda:
        ...
        if self.savegrad:
            ...
            file_name = str(now.day) + '-' + str(now.month) + '-' + str(now.year) + '_' + \
                        str(now.hour) + '-' + str(now.minute) + '-' + str(now.second)
            torch.save(gradients, 'grad_' + file_name)
            torch.save(self.optimizer.state_dict(), 'optimizer_' + file_name)
            torch.save(weights, 'weights_' + file_name)
